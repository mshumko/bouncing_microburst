...
fig = plt.figure(figsize=(15, 15), dpi=80, facecolor = 'white')
...
for i in range(4):
    ...
    ax[i].plot(fitObj3.times[bounceInd3] +
    ...
    # Plot FU4 col Flux
    ax[i].plot(fitObj4.times[bounceInd4],
...
ax[0].set(title = 'Bouncing packet microburst observed with the FIREBIRD-II collimated detector', ylabel = r'Flux $(s \ cm^2 \ sr \ keV)^{-1}$')

# Set plot x and y axis limits.
for i in ax:
    i.xaxis.set_major_formatter(hfmt)
    i.legend(loc = 'upper left', fontsize = 12)
    i.set(ylabel = r'Flux $(s \ cm^2 \ sr \ keV)^{-1}$')
    i.set_ylim([0.01, 30])
    i.set_yscale('log')
    i.set_xlim([datetime.datetime(2015, 2, 2, 6, 
