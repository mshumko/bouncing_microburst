...
fluxPlt = fig.add_subplot(gs[0, 0])
...
for i in range(4):
    ...
    fluxPlt.plot(fitObj3.times[bounceInd3] +
    datetime.timedelta(seconds = secOffset),
    smoothedFlux3(i, roll_avg), '-', c = c[i],
    label = ('%.0f' % FU3ColBounds[i] + r'$ < E_{FU3} < $' + 
    '%.0f' % FU3ColBounds[i+1]), linewidth = 3)

    # Plot FU4 col Flux
    fluxPlt.plot(fitObj4.times[bounceInd4],
    smoothedFlux4(i, roll_avg), '--', label = ('%.0f' %
    FU4ColBounds[i] + r'$ < E_{FU4} < $' + '%.0f' % 
    FU4ColBounds[i+1]), linewidth = 3, c = c[i])
        
fluxPlt.set_yscale('log')

# Format the time stamps 
hfmt = dates.DateFormatter('%H:%M:%S')
fluxPlt.xaxis.set_major_formatter(hfmt)

# Legend
fluxPlt.legend(loc = 'upper left', fontsize = 12)

# Set the title, and axies labels.

fluxPlt.set(title = 'Bouncing packet microburst observed with FIREBIRD-II Surface detectors', ylabel = r'Flux $(s \ cm^2 \ sr \ keV)^{-1}$', xlabel = 'UTC')
    

# Set plot x and y axis limits.
fluxPlt.set_ylim([0.01, 30])
#if plotSur:
#    surPlt3.set_ylim([0, 30])
fluxPlt.set_xlim([datetime.datetime(2015, 2, 2, 6, 
12, 50, 0), datetime.datetime(2015, 2, 2, 6, 12, 
56, 500000)])

# Grid settings
fluxPlt.minorticks_on()
